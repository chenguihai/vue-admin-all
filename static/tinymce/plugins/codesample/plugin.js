                  ...
                  matchGrammar: function (text, strarr, grammar, index, startPos, oneshot, target) {
                    for (var token in grammar) {
                      ...
                      for (var j = 0; j < patterns.length; ++j) {
                        ...
                        var pattern = patterns[j], inside = pattern.inside, lookbehind = !!pattern.lookbehind,
                          greedy = !!pattern.greedy, lookbehindLength = 0, alias = pattern.alias;
                        ...
                        for (var i = index, pos = startPos; i < strarr.length; pos += strarr[i].length, ++i) {
                          ...
                          if (greedy && i != strarr.length - 1) {
                            ...
                            var from = match.index + (lookbehind && match[1] ? match[1].length : 0),
                              to = match.index + match[0].length, k = i, p = pos;
                          ...
                          var from = match.index + lookbehindLength, match = match[0].slice(lookbehindLength),
                            to = from + match.length, before = str.slice(0, from), after = str.slice(to);
                          var args = [
                            i,
                            delNum
                          ];
